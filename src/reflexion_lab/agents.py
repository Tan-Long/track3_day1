    ...
    def run(self, example: QAExample) -> RunRecord:
        ...
        for attempt_id in range(1, self.max_attempts + 1):
            ...
            if self.agent_type == "reflexion" and attempt_id < self.max_attempts:
                entry, ref_tokens, ref_latency = reflector(example, attempt_id, judge)
                reflections.append(entry)
                token_estimate += ref_tokens
                latency_ms += ref_latency
                trace = AttemptTrace(attempt_id=attempt_id, answer=answer, score=judge.score, reason=judge.reason, reflection=entry, token_estimate=token_estimate, latency_ms=latency_ms)
                reflection_memory.append(f"Attempt {entry.attempt_id} failed. Lesson: {entry.lesson} Strategy: {entry.next_strategy}")
